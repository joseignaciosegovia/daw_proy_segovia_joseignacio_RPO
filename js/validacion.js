...
function validarEditarGestor(form) {
  // Deshabilitamos la forma declarativa de validación
  ...
  form.addEventListener('submit', function (event) {
    ...
    const ncFoto = document.getElementById('foto');
      ...
      if (!isValid) {
        ...
        // Al llamar "Actualizar" al parámetro del "formData" que enviamos al servidor,
        // éste accederá a su contenido (es decir, "datosAEnviar") con "$_POST['Actualizar']"
  
        formData.append("Actualizar", datosAEnviar);
        ...
        fetch('editarGestor.php', {
          method: 'post',
          body: formData
        }).then((response) => response.text())
        .then(function(data) {
          mostrarModal("El gestor ha sido modificado correctamente", "/servidor/editarGestor.php");
